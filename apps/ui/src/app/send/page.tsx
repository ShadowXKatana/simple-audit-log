...
import { sendAuditEvent } from '@/lib/api';
import {
    AuditEvent,
    ACTION_TYPES,
    ROLES,
...
export default function SendEventPage() {
    ...
    const [result, setResult] = useState<{ log_id: string; offset: number } | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Add changes/payload if provided
        ...
        try {
            const res = await sendAuditEvent(event);
            setResult(res);
        } catch (err) {
                    ...
                    <div className="flex flex-wrap gap-2">
                        {ACTION_TYPES.map((a) => (
                            <button
                                ...
                                type="button"
                                onClick={() => setAction(a)}
                                className={`px-4 py-2 text-sm font-medium rounded-lg border transition-all duration-200 ${action === a
                ...
                {(showChanges || showPayload) && (
                    <div className="glass-card p-5 space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label className="block text-xs text-muted-foreground mb-1">Old Value</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs text-muted-foreground mb-1">New Value</label>
                                <input
                                    type="text"
            ...
            {result && (
                <div className="glass-card p-5 border-emerald-500/30 bg-emerald-500/5 animate-fade-in">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-emerald-400">Event Sent Successfully</p>
                            <div className="mt-2 space-y-1">
                                <p className="text-xs text-muted-foreground">
                                    <span className="text-foreground font-mono">{result.log_id}</span>
                                </p>
                                <p className="text-xs text-muted-foreground">
                                    Kafka offset: <span className="text-foreground font-mono">{result.offset}</span>
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            {error && (
                <div className="glass-card p-5 border-red-500/30 bg-red-500/5 animate-fade-in">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-red-400">Failed to Send Event</p>
